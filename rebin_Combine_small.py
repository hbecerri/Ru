...
channel = ['SR1T','SR0T','CR1','CR2']
proc = ['QCD','wjets_b','wjets_c','DY']
fout = TFile('/afs/desy.de/user/h/hugobg/public/theta/CMSSW_8_0_24/src/Combine_Limits/muon_mle/rebin_small_Combine.root', 'recreate')
...
for ch in channel:
    ...
    if ch == 'SR1T':
        binning1 = array.array('d', computeBinning(tempdata1, 0.25))
    elif ch == 'SR0T':
        binning2 = array.array('d', computeBinning(tempdata2, 0.25))
    elif ch == 'CR1':
        binning3 = array.array('d', computeBinning(tempdata3, 0.25))
    elif ch == 'CR2':
        binning4 = array.array('d', computeBinning(tempdata4, 0.25))
    print(binning1)

    for pr in proc:
        if ch == 'SR1T':
            for hkey in list_syst[ch+pr]:
                myfile = TFile('mu_Test_4cat.root')
                gDirectory.cd('mu_Test_4cat.root:/'+ch+'/'+pr)
		print(ch+'/'+pr+'_'+hkey)
                myhisto = gDirectory.Get(hkey).Clone()
#                myhisto = myhisto.Rebin(len(binning1)-1, 'myhisto', binning1)
                myhisto = myhisto.Rebin(4, 'myhisto')
                fout.cd(ch+'/'+pr)
                myhisto.Write(hkey)
		del myhisto
            ...
            for hkey in list_syst[ch+pr]:
                ...
                gDirectory.cd('mu_Test_4cat.root:/'+ch+'/'+pr)
                myhisto = gDirectory.Get(hkey).Clone()
#                myhisto = myhisto.Rebin(len(binning2)-1, 'myhisto', binning2)
       	       	myhisto	= myhisto.Rebin(4, 'myhisto')
                fout.cd(ch+'/'+pr)
                myhisto.Write(hkey)
        elif ch == 'CR1':
...
#                myhisto = myhisto.Rebin(len(binning3)-1, 'myhisto', binning3)
            ...
            for hkey in list_syst[ch+pr]:
                ...
                gDirectory.cd('mu_Test_4cat.root:/'+ch+'/'+pr)
                myhisto = gDirectory.Get(hkey).Clone()
#                myhisto = myhisto.Rebin(len(binning4)-1, 'myhisto', binning4)
      	       	myhisto	= myhisto.Rebin(4, 'myhisto')
                fout.cd(ch+'/'+pr)
                myhisto.Write(hkey)
