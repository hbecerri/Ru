...
fout = TFile('/afs/desy.de/user/h/hugobg/public/theta/CMSSW_8_0_24/src/Combine_Limits/muon_mle/rebin_q2_SR1T.root', 'recreate')
...
for ch in channel:
    for pr in proc:
        if ch == 'SR1T':
                myfile = TFile('mu_Test_4cat_q2_SR1T.root')
                gDirectory.cd('mu_Test_4cat_q2_SR1T.root:/'+ch+'/'+pr)
